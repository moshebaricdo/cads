...
import type { PaginationProps, LabelCompactPagesArgs } from "./types";
  ...
  function Pagination(
    ...
    const renderItem = (item: PaginationRenderItemParams) => {
      ...
      if (ellipsis) {
        return (
          <div
            className={`CadsPagination-item CadsPagination-ellipsis ${moduleStyles.ellipsis}`}
            aria-hidden
            data-disabled={isDisabled ? "true" : undefined}
            style={{ "--pg-height": dims.height } as CSSProperties}
          >
      ...
      /* MUI ButtonBase sx: targets MUI pseudo-class selectors (.Mui-focusVisible,
         .Mui-disabled) that can't be targeted from CSS modules. */
      const sx = {
    ...
    /* MUI Pagination sx: targets MUI internal class selectors
       (.MuiPagination-ul, li:first-of-type, li:last-of-type) for the
       segmented-button pill shape, which can't be done via CSS modules. */
    const segmentedSx = {
      display: "inline-flex",
    ...
    const clusterVars = {
      "--pg-cluster-gap": clusterDims.clusterGap,
      "--pg-cluster-height": clusterDims.height,
      "--pg-cluster-font-size": clusterDims.fontSize,
      "--pg-cluster-line-height": clusterDims.lineHeight,
    } as CSSProperties;
    ...
    const compactCluster = (
      <div
        ...
        className={moduleStyles.compactCluster}
        style={clusterVars}
      >
        <Button
          variant="outlined"
        ...
        />
        <span className={moduleStyles.compactLabel}>
    ...
    return (
      ...
      >
        {layout === "auto" && showCompact ? (
          <div aria-hidden className={moduleStyles.measureHidden}>
            <div ref={measureRef} className={moduleStyles.measureInner}>
